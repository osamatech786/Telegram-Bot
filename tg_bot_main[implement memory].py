...
# chain = LLMChain(llm=llm, prompt=prompt)  # Since we are using agents, we don't need the chain here
...
# Since we are using main agent as a router, we don't need to route queries manually
...
# Memory Key differences:
# =======================
"""
1/3; Buffer Memory: Temporary, recent interactions, FIFO (Stores raw conversation data in short-term memory)
...
buffer_memory = ConversationBufferMemory(
    ...
    k=5  # Keeps last 5 interactions
)
```

2/3; Context Memory: Semantic understanding, summarized information, token-aware

code:
```python
from langchain.memory import ConversationSummaryMemory

context_memory = ConversationSummaryMemory(
    llm=llm,
    memory_key="chat_history",
    max_token_limit=200  # Maintains context within token limit
)
```

3/3; Persistent Memory: Long-term storage, survives restarts, external storage
...
sql_persistent_memory = ConversationBufferMemory(
    memory_key="chat_history",
...
Use Buffer Memory for simple chatbots needing only recent context
Use Context Memory for more sophisticated conversations requiring understanding
Use Persistent Memory when you need to maintain history across sessions

Or You can combine these different types of memory based on your needs:
Code:
```python
...
combined_memory = CombinedMemory(
    memories=[
        buffer_memory,      # For immediate context
        context_memory,     # For maintaining conversation context
        persistent_memory   # For long-term storage
    ]
)
